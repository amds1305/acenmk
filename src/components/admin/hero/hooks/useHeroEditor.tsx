...
export const useHeroEditor = () => {
  ...
  const [activeTab, setActiveTab] = useState<'versions' | 'carousel' | 'blocks'>('versions');
  ...
  const { updateExistingSectionData, saveChanges } = useSections();

  // Récupérer la configuration existante du Hero
  const { data: homeConfig, isLoading, refetch } = useQuery({
    queryKey: ['homeConfig'],
  ...
  useEffect(() => {
    if (homeConfig?.sectionData?.hero) {
      ...
      console.log('Loading hero settings from config:', existingHero, heroData);
  ...
  const saveHeroChanges = async () => {
    ...
    try {
      ...
      // Mettre à jour les données de base du hero
      await updateExistingSectionData('hero', {
      ...
      // Mettre à jour les paramètres avancés du hero
